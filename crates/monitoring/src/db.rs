...
use crate::models::{DayBucket, HourBucket, QueryStats, SearchQueryRow, TopQuery};
    ...
    pub async fn get_hourly_histogram(
        pool: &sqlx::Pool<sqlx::Sqlite>, hours: i64) -> Result<Vec<HourBucket>> {
        let rows = sqlx::query(
    ...
    pub async fn get_daily_histogram(
        pool: &sqlx::Pool<sqlx::Sqlite>, days: i64) -> Result<Vec<DayBucket>> {
        let rows = sqlx::query(
    ...
    pub async fn get_zero_result_queries(
        pool: &sqlx::Pool<sqlx::Sqlite>, limit: i64) -> Result<Vec<TopQuery>> {
        let rows = sqlx::query(
    ...
    pub async fn get_error_count(
        pool: &sqlx::Pool<sqlx::Sqlite>, since_hours: i64) -> Result<i64> {
