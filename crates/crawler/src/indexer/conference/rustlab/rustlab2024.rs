...
static RUSTLAB_2024_BASE_URL: LazyLock<Url> = LazyLock::new(|| static_url("http://web.archive.org/web/20241119164137/https://rustlab.it/"));
static RUSTLAB_2024_PLAYLIST_URL: LazyLock<Url> =
...
impl ScheduleParser for RustLab2024 {
    fn metadata(&self) -> ConferenceMetadata {
        ConferenceMetadata {
            ...
            youtube_playlist_url: Some((*RUSTLAB_2024_PLAYLIST_URL).clone()),
        }
    }

    async fn parse(&self, client: &reqwest::Client) -> Result<Vec<ParsedTalk>> {
        let base_url = &*RUSTLAB_2024_BASE_URL;
    ...
    fn parse_schedule(&self, document: &Html, base_url: &Url) -> Result<Vec<ParsedTalk>> {
        ...
        for day in days {
            ...
            let date = NaiveDate::parse_from_str(date_str, "%Y-%m-%d")
                .unwrap_or_else(|_| NaiveDate::from_ymd_opt(2024, 11, 1).unwrap());
            ...
            if let Some(schedule) = day.get("schedule").and_then(|v| v.as_array()) {
                for event in schedule {
                    ...
                    debug!(
                        "Parsed RustLab 2024 talk: {} ({} speakers)",
                        title,
                        speakers.len()
                    );
        ...
        if talks.is_empty() {
            log::warn!(
                "No talks found in RustLab 2024 schedule data yet (might be too early for the schedule to be released)."
            );
        } else {
...
mod tests {
    ...
    #[test]
    fn test_rustlab_2024_metadata() {
        let parser = RustLab2024;
        let metadata = parser.metadata();
        assert_eq!(metadata.id, "rustlab-2024");
        assert_eq!(metadata.conference, "RustLab");
        assert_eq!(metadata.year, "2024");
        assert_eq!(
            metadata.url,
            Url::parse("http://web.archive.org/web/20241119164137/https://rustlab.it/").expect("valid RustLab 2024 base URL")
        );
        assert_eq!(
            metadata.youtube_playlist_url,
            Some(
                Url::parse("https://www.youtube.com/@rustlabconference3671")
                    .expect("valid RustLab 2024 playlist URL")
            )
        );
    }
}
