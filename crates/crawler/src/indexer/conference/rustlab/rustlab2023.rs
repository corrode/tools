...
#[async_trait]
impl ScheduleParser for RustLab2023 {
    fn metadata(&self) -> ConferenceMetadata {
        ConferenceMetadata {
            id: "rustlab-2023",
            conference: "RustLab",
            year: "2023",
            url: (*RUSTLAB_2023_BASE_URL).clone(),
            youtube_playlist_url: Some((*RUSTLAB_2023_PLAYLIST_URL).clone()),
        }
    }

    async fn parse(&self, client: &reqwest::Client) -> Result<Vec<ParsedTalk>> {
        let base_url = &*RUSTLAB_2023_BASE_URL;
    ...
    fn parse_schedule(&self, document: &Html, base_url: &Url) -> Result<Vec<ParsedTalk>> {
        ...
        for day in days {
            ...
            let date = NaiveDate::parse_from_str(date_str, "%Y-%m-%d")
                .unwrap_or_else(|_| NaiveDate::from_ymd_opt(2023, 11, 1).unwrap());
            ...
            if let Some(schedule) = day.get("schedule").and_then(|v| v.as_array()) {
                for event in schedule {
                    ...
                    debug!(
                        "Parsed RustLab 2023 talk: {} ({} speakers)",
                        title,
                        speakers.len()
                    );
        ...
        if talks.is_empty() {
            tracing::warn!(
                "No talks found in RustLab 2023 schedule data yet (might be too early for the schedule to be released)."
            );
        } else {
...
mod tests {
    ...
    #[test]
    fn test_rustlab_2023_metadata() {
        let parser = RustLab2023;
        let metadata = parser.metadata();
        assert_eq!(metadata.id, "rustlab-2023");
        ...
        assert_eq!(
            ...
            Some(
                Url::parse("https://www.youtube.com/@rustlabconference3671")
                    .expect("valid RustLab 2023 playlist URL")
            )
        );
    }
}
